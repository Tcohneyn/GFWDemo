...
public class lanToggleGroupManager : MonoBehaviour
{
    [Header("组件引用")] [SerializeField] private ToggleGroup toggleGroup; // 拖拽赋值
    [SerializeField] private bool lockSelectedToggle = true; // 核心：选中后锁定

    [Header("外观 (可选)")] [SerializeField] private Color normalColor = Color.white;
    [SerializeField] private Color selectedLockedColor = new Color(0.6f, 0.8f, 1.0f, 0.8f); // 选中锁定后的颜色

    private Dictionary<Language, Toggle> allToggles = new Dictionary<Language, Toggle>();
    ...
    {
        // 为每个 Toggle 绑定事件
        foreach (KeyValuePair<Language, Toggle> toggle in allToggles)
        {
            if (toggle.Value != null)
            {
                // 绑定值变化事件
                toggle.Value.onValueChanged.AddListener((isOn) => OnToggleValueChanged(toggle.Value, isOn));
                mModel.mCurrentLanguage.RegisterWithInitValue(language =>
                {
                    //LocaleKit.ChangeLanguage(language);
                    if (toggle.Key == language)
                    {
    ...
    private void CollectAllToggles()
    {
        ...
        foreach (Toggle t in toggles)
        {
            if (t.group == toggleGroup) // 确保只管理本组的
            {
                if (t.name == "cnToggle")
                {
                    allToggles.Add(Language.ChineseSimplified, t);
                }
                else if (t.name == "enToggle")
                {
                    allToggles.Add(Language.English, t);
                }
                else if (t.name == "jpToggle")
                {
                    allToggles.Add(Language.Japanese, t);
                }
            }
        }
    }
